 ...
 *     (Apple Pay with a saved card, Google Pay with a saved card, or Link logged in).
...
interface ProductExpressCheckoutProps {
   ...
   * Use this in the parent to hide separators / labels when nothing is available.
   */
...
function PaymentRequestInner({
  ...
  // Missing-phone fallback (Shopify-style)
  ...
  const allowedCountryCodes = useMemo<string[]>(() => {
    const v2: any[] = (shippingCoverageV2 as any)?.countries || []
  ...
  useEffect(() => {
    ...
    pr.canMakePayment().then((result) => {
      ...
      // Only show the button for real authenticated wallets (Apple Pay / Google Pay).
      // Intentionally skip Link: Stripe reports it as available even in incognito
      // (guest checkout), which would add noise on PDP and dilute the primary CTA.
      const hasRealWallet = !!(result?.applePay || result?.googlePay)
      console.log('[PDP ExpressCheckout] hasRealWallet:', hasRealWallet)
      if (hasRealWallet) {
  ...
  // Keep total in sync when quantity / price changes
  useEffect(() => {
  ...
  // Validate shipping country and recalculate totals when wallet picks a shipping option
  useEffect(() => {
    if (!paymentRequest) return
  ...
  useEffect(() => {
    ...
    const handlePaymentMethod = async (ev: any) => {
      try {
        ...
        const shippingAddress =
          shipping?.recipient || shipping?.addressLine
            ? {
                first_name: (shipping.recipient || payerName || '').split(' ')[0] || '',
                last_name:
                  (shipping.recipient || payerName || '').split(' ').slice(1).join(' ') || '',
        ...
        const order = await createCheckoutFromCart(
          ...
          pendingDiscount,
          shippingAddress,
          shippingAddress, // billing same as shipping for express checkout
          undefined,
          currencyCode,
        )
        ...
        const intentPayload = {
          ...
          delivery_fee: order.order?.shipping_amount
            ? Math.round((order.order.shipping_amount as number) * 100)
            : 0,
        ...
        // 5. Confirm the PaymentIntent with the wallet's PaymentMethod
    ...
    paymentRequest, stripe, product, variant, sellingPlan, quantity, unitPrice,
    currencyCode, clearCart, navigate, toast, requestMissingPhone,
  ])
...
export default function ProductExpressCheckout(props: ProductExpressCheckoutProps) {
  ...
  // Lazy-mount Stripe when the component scrolls into view
  useEffect(() => {
  ...
  const stripePromise = useMemo<Promise<Stripe | null>>(() => {
    const opts =
      chargeType === 'direct' && stripeAccountId
