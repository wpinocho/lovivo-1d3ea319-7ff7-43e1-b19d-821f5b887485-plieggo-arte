 ...
 * 
 * Este componente contiene toda la lógica de negocio de productos:
...
export const useProductCardLogic = (product: Product) => {
  ...
  const handleOptionChange = (optName: string, value: string) => {
    setSelected((prev) => {
  ...
  return {
    ...
    onAddToCartSuccess: () => {
      console.log('Product added to cart - ready for additional features like confetti')
    }
  }
}

interface HeadlessProductCardProps {
  product: Product
  children: (logic: ReturnType<typeof useProductCardLogic>) => React.ReactNode
}

export const HeadlessProductCard = ({ product, children }: HeadlessProductCardProps) => {
  const productLogic = useProductCardLogic(product)
