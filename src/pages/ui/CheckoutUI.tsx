              ...
              <div className="space-y-8 bg-card p-3 sm:p-6 rounded-lg">
                ...
                <section>
                  {(() => {
                    ...
                    // "Best of both worlds": el checkout monta COMPLETO desde el inicio
                    // en modo deferred (correo vía LinkAuthenticationElement con autollenado
                    // de Stripe Link, dirección, pago y express). En cuanto el correo es
                    // válido, StripePayment crea el intent up-front y remonta Elements en
                    // modo client_secret → aparece el selector de meses sin intereses.
                    // El correo sobrevive el remonte porque se re-inyecta vía
    ...
    <div className="pt-2 border-t">
      {!logic.discount ? (
        ...
        <div className="flex items-center justify-between text-xs bg-muted/50 border border-border p-2 rounded-lg">
          <span className="text-foreground font-medium">Cupón: {logic.couponCode}</span>
          <Button variant="ghost" size="sm" onClick={logic.removeCoupon} className="text-muted-foreground hover:text-foreground p-1 h-auto">
            <X className="h-3 w-3" />
          </Button>
        </div>
