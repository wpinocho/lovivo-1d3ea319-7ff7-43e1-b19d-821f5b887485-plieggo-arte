...
- **Menú global** en `src/templates/EcommerceTemplate.tsx` (desktop array + mobile Sheet). Incluye ahora `/personalizados` ("Personalízalo", destacado en primary).
- **Imágenes producto son 4:5** (portrait). Usar `aspect-[4/5]` en cards/galerías, NUNCA aspect-square (corta).

## 3. Active Plan — landing /personalizados AJUSTADA a template global ✅

### Landing /personalizados — CONSTRUIDA + AJUSTADA ✅ (2026-07-09)
- Ruta `/personalizados` en `src/App.tsx` (lazy). Página: `src/pages/Personalizados.tsx`.
- **AHORA usa `<EcommerceTemplate>`** (menú global + AnnouncementBar + footer). Se eliminó el header minimal propio. Mantiene sticky mobile CTA bottom y evento Lead.
- **En el menú** (desktop + móvil) como "Personalízalo".
- Hero image = 2º slide del home ("Encuentra tu pieza perfecta"): `message-images/.../1779296069343-2ifge8n87sv.webp`.
- Aspect ratios corregidos a `aspect-[4/5]` (galería + sección luz). Cards de estilo ya estaban 4:5.
- **Evento Lead Meta**: `facebookPixel.lead()` en TODOS los CTA WhatsApp.
- NO menciona restricciones (paleta/100×70/lino/acrílico) — se filtra en WhatsApp.

### PENDIENTE DE VALIDAR (próxima sesión)
- Screenshot mobile de `/personalizados` (staging no refleja en mismo run).
- Confirmar en prod que `fbq('track','Lead')` dispara al clic.

## 4. Recent Changes
- **2026-07-09** — ✅ LANDING `/personalizados` AJUSTADA: envuelta en `EcommerceTemplate` (menú global + banner + footer), agregada al menú desktop+móvil ("Personalízalo"), hero cambiado al 2º slide del home, aspect ratios a 4:5 (galería + sección luz). Eliminado header minimal propio.
