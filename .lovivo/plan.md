...
## 3. Active Plan
**Sin plan activo.** "Arte vivo" honesto ya implementado. Siguiente foco sugerido: medir ATC de best-sellers (triptych) vs resto (single).

## 4. Recent Changes
- **2026-07-07** — ✅ "Arte vivo" honesto implementado. `LightShadowFeature.tsx` ahora tiene prop `variant`: `triptych` (comportamiento actual, 3 fotos) y `single` (layout 2-col: 1 foto lifestyle grande aspect-[4/5] + copy honesto "Luz y sombra que cobran vida", sin prometer 3 momentos). `ProductPageUI.tsx`: si hay set real → triptych; si no → single con `product.images[1]` (fallback [0], si nada → null). Se acabaron los rótulos falsos de 3 horas en los 14 no-best-sellers.
- **2026-07-07** — 🔎 Decisión "Arte vivo" no-best-sellers: NO simular luz con filtro CSS (se ve falso). Plan: variante `single` para los 14; triptych real solo en best-sellers. Piloto limpio.
- **2026-07-07** — ✅ ProductCard estandarizado a 4:5 en TODAS las páginas (ProductCardUI ignora prop aspectRatio, usa aspect-[4/5] + object-cover). Skeletons a 4:5 en Index/TopSellers/AllProducts/CollectionAcordeon/CollectionEspacio. Hover ahora SIEMPRE muestra product.images[1] (2ª imagen general).
- **2026-07-07** — ✅ "Arte vivo" REAL en 2 best-sellers. Creado `src/data/light-shadow-sets.ts` (map slug→[mañana, tarde, atardecer]). Generadas 4 imágenes (gemini + reference lifestyle del dueño). Pendiente aprobación dueño.
...
- **[ALTA]** Dueño: aprobar las 4 imágenes de "Arte vivo" (triptych) en verde-salvia y beige-sutil.
- **[ALTA]** Verificar en preview que la variante `single` se vea premium en móvil (ej. Luna Beige / Burdeos).
