...
- **GALERÍA PDP — REGLA (fix 2026-07-08)**: `getDisplayImages()` en `HeadlessProduct.tsx` ahora MERGEA `product.images` (siempre, orden dashboard) + extras de variante deduped. Antes devolvía SOLO `variant.image_urls`, lo que ocultaba imágenes del dashboard y mostraba primero una imagen de variante distinta/no-4:5.
- **"Arte vivo" (LightShadowFeature)**: variantes `triptych` (best-sellers) y `single` (resto).
- **CHECKOUT trust signals**: `src/components/CheckoutTrustBadges.tsx` (banner SSL, rating real, garantías, logos pago). Iconos SVG line terracota, sin emojis, sin glow.
...
Intento Craft Mode aplicado en `StripePayment.tsx buildPayload` (`payment_method_options.card.installments.enabled=true`). **PENDIENTE VERIFICAR EN PREVIEW** con tarjeta MX elegible. Si no aparecen → edge no soporta installments (escalado a Lovivo).
...
Product images del dashboard ahora SIEMPRE en el carrusel. Corrige "falta imagen 2" e imagen equivocada/cortada en verde-salvia.

## 4. Recent Changes
- **2026-07-08** — ✅ FIX galería PDP: `getDisplayImages()` mergea product.images + variantes (antes devolvía solo variant.image_urls → ocultaba imágenes del dashboard y verde-salvia mostraba una imagen de variante no-4:5 cortada). Corregido también trust strip PDP "10–15 días" → "Entrega 5–7 días hábiles".
- **2026-07-08** — ✅ Fix checkout: miniaturas resumen 4:5 (móvil w-12 h-[60px], desktop w-16 h-20). Envío en resumen móvil muestra "GRATIS" siempre.
- **2026-07-08** — ✅ CHECKOUT CRO. Nuevo `CheckoutTrustBadges.tsx`. Integrado en `StripePayment.tsx` (banner, rating, garantías, logos, fix business.name→'Plieggo Arte', MSI vía payment_method_options). Resumen móvil abierto + estimación 5-7 días.
...
- **DATO CLAVE**: varios productos tienen `variant.image_urls` que NO coinciden 1:1 con `product.images` (imágenes extra o distintas, algunas no-4:5, p.ej. verde-salvia `cydjtdr71j7`/`r449lwaje3h`). El fix de galería garantiza que product.images siempre lidere. Si el dueño quiere que TODO sea 4:5, limpiar esas variant-only en el dashboard.
- **verde-salvia** (id 16782cd1-...): product.images[0]=aic3ta4yru (4:5, imagen correcta del dashboard).
- **Faltan reseñas (fotos)**: Beige Sutil y Luna Beige — el dueño las subirá.

## 6. Known Issues
- **MSI depende de edge function Lovivo** (`payments-create-intent`). VERIFICAR en preview.
- **Imágenes variant-only no-4:5** (ej. verde-salvia): con el fix ya no lideran, pero pueden aparecer en posiciones posteriores del carrusel y verse recortadas (object-cover). Recomendar al dueño depurarlas en el dashboard.
- **Verificar tarifa de envío Dashboard = $0 todo México** para no contradecir copy.
...
- **[ALTA]** VERIFICAR en preview tras deploy: (a) galería PDP muestra todas las imágenes del dashboard y verde-salvia ya no se corta; (b) MSI con tarjeta MX; (c) trust badges premium en móvil.
- **[ALTA]** Verificar tarifa envío Dashboard = $0.
- **[MEDIA]** Sugerir al dueño limpiar imágenes variant-only no-4:5 en dashboard (o consolidar todo en product.images).
- **[ALTA]** Dueño: aprobar las 4 imágenes de "Arte vivo" (triptych).
- **[MEDIA]** Si MSI funcionan: micro-copy "3/6/12 meses sin intereses" en PDP.
