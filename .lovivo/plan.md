...
- **Menú principal (EcommerceTemplate)**: orden desktop+móvil = Todos los Cuadros, Más Vendidos, Acordeón, Espacio, Galería, Nosotros, Rastrear pedido. Footer "Navegación" incluye Inicio, Sobre Nosotros, Rastrear pedido.

## 3. Active Plan
**Estado:** ✅ COMPLETADO — Página pública de rastreo de pedidos (Order Tracking)

Backend `order-track` ya desplegado (NO tocado). Frontend del template construido en esta sesión.

### Implementado
- `src/pages/OrderTrack.tsx` — wrapper delgado: lee `:token`, inyecta `<meta robots="noindex,nofollow">` vía useEffect (cleanup al desmontar), monta `OrderTrackUI` dentro de EcommerceTemplate.
- `src/pages/ui/OrderTrackUI.tsx` — UI completa (Tipo B). Dos modos:
  - Modo token: `callEdge('order-track', { token })` al montar.
  - Modo lookup (`/orders/track` sin token): form order_number + email → `callEdge('order-track', { store_id: STORE_ID, order_number, email })`.
  - Timeline horizontal 4 pasos (Confirmado/Preparando/Enviado/Entregado), círculos completado/actual/pendiente + conectores, fecha bajo cada paso (date-fns locale es).
  - Banner rojo si `cancelled`. Bloque "Entrega estimada" (primary/terracota). Bloque carrier + tracking copiable + botón "Rastrear con la paquetería" (SOLO si `display_mode !== 'masked'`). Eventos en Accordion (SOLO detailed).
  - Estados: loading skeleton, error "No encontramos tu pedido".
  - Parsing defensivo: lee steps/current_step/cancelled desde `data.timeline` O top-level.
- `src/App.tsx` — rutas lazy `/orders/track` y `/orders/track/:token`.
- `src/lib/supabase.ts` — `interface Order` extendida: tracking_number, tracking_url, shipping_carrier, estimated_delivery_at, shipped_at, paid_at.
- `src/pages/ui/MyOrdersUI.tsx` — dentro de cada card: línea "Entrega estimada", botón terracota "Rastrear pedido" → `/orders/track/{checkout_token}`, chip guía + link externo a la paquetería.
- `src/pages/ThankYou.tsx` — `OrderDetails` con `checkout_token`; CTA "Rastrear mi pedido" en action buttons si está presente.
- `src/templates/EcommerceTemplate.tsx` — "Rastrear pedido" en menú desktop + móvil + footer.

## 4. Recent Changes
- **2026-06-24** — ✅ Order Tracking COMPLETO: OrderTrack.tsx + OrderTrackUI.tsx (timeline/lookup/carrier/eventos), rutas /orders/track(/:token), Order interface extendida, CTA en MyOrders + ThankYou, "Rastrear pedido" en menú y footer.
...
## 6. Known Issues
- **Confirmar que `completed_order` en localStorage incluya `checkout_token`** — el CTA "Rastrear mi pedido" de ThankYou solo aparece si el campo existe. Viene en `CheckoutResponse.checkout_token`; revisar StripePayment/checkout flow donde se guarda `completed_order` para garantizar que se persista.
- Verificar shape real del response `order-track` (steps[] keys, si current_step/cancelled van en `timeline` o top-level). OrderTrackUI ya parsea ambos casos, pero validar en producción con un pedido real enviado.
...
- **[ALTA]** Verificación end-to-end Order Tracking: generar etiqueta Envia → email order_shipped con link `/orders/track/{token}` → abrir → ver timeline/carrier/tracking/entrega estimada → simular webhook → recargar; probar lookup sin token; probar `tracking_display_mode='masked'`.
- **[ALTA]** Garantizar `checkout_token` en `completed_order` (localStorage) para el CTA de ThankYou.
- **[ALTA]** Performance móvil: mover fuentes Google a HTML, lazy-load InspirationCarousel, fetchpriority en hero image
